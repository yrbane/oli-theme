...
final class AssetManager
{
     ...
     *
     * Deux sources de priorité décroissante :
     *  1. `oli_internal_banner_image` : URL unique configurée sur l'écran
     *     Apparence → Variations CSS.
     *  2. Image par défaut du thème (codée dans la variation CSS active).
     */
    private function injectInternalBannerOverride(string $handle): void
    {
        ...
        $url = \function_exists('esc_url') ? (string) esc_url($url) : $url;
        if ($url === '') {
            return;
        }

        wp_add_inline_style($handle, \sprintf("html{--oli-internal-banner-url:url('%s');}", $url));
    }
